      ...
      if (config.apiProvider === "openai") {
        ...
        if (config.apiKey) {
          this.openaiClient = new OpenAI({
            ...
            defaultHeaders: {
              "HTTP-Referer":
                "https://github.com/Suryanshu-Nabheet/Intervue_Coder",
              "X-Title": "Intervue Coder",
            },
          });
          console.log("OpenRouter client initialized successfully");
        } else {
          console.warn(
            "No API key available, OpenRouter client not initialized",
          );
        }
      } else if (config.apiProvider === "ollama") {
        // Ollama usually doesn't need a key, just base URL
      ...
      if (
        ...
        if (!this.openaiClient) {
          ...
          if (!this.openaiClient) {
            return {
              success: false,
              error: `${config.apiProvider} not configured properly. Please check your settings.`,
            };
          }
        }

        // Use OpenAI/OpenRouter/Ollama for processing
        const messages = [
          {
        ...
        // Send to OpenAI Vision API or compatible
...
`;
      ...
      if (
        ...
        if (!this.openaiClient) {
          return {
            success: false,
            error: `${config.apiProvider} not configured. Please check your settings.`,
          };
        }
